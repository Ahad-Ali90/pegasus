  ...
  <span class="slider">
   <svg class="slider-icon" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="presentation"><path fill="none" d="m4 16.5 8 8 16-16"></path></svg> 
    ...
    <div>
        <button onclick="openAddModal()"
            ...
            + Add CarFeature
                        ...
                        <table class="min-w-full divide-y divide-gray-200 border"  style="width:100%">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Luggage Type</th>
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Max Dimensions Cm</th>
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Max Dimensions Inches</th>
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200 text-center" id="sortableBody">
                                @foreach($LuggageGuidelines as $index => $LuggageGuideline)
                                    <tr class="text-center" data-id="{{ $LuggageGuideline->id }}">
                                        <td class="px-6 py-2">{{ $loop->iteration }}</td>
                                        <td class="px-6 py-2">{{ $LuggageGuideline->car->name }}</td>
                                        <td class="px-6 py-2">{{ $LuggageGuideline->type }}</td>
                                        <td class="px-6 py-2">{{ $LuggageGuideline->max_dimensions_cm }}</td>
                                        <td class="px-6 py-2">{{ $LuggageGuideline->max_dimensions_inches }}</td>
                                        ...
                                        <button 
                                            type="button"
                                            onclick='openEditModal(@json($LuggageGuideline))'
                                            class="px-3 py-1 text-sm text-black bg-green-500 rounded hover:bg-green-600">
                                            Edit
                                        </button>
                                        {{-- Delete Button --}}
                                        <form action="{{route('LuggageGuideline.destroy',$LuggageGuideline->id)}}" 
                                            method="post" 
                                            class="inline-block ajax-delete-form"
                                            data-id="{{ $LuggageGuideline->id }}">
                                            @csrf
                                            @method('DELETE') 
                    ...
                    @if ($LuggageGuidelines->hasPages())
                        <div class="mt-4 bg-white p-4 rounded shadow">
                            {{ $LuggageGuidelines->links() }}
                        </div>
                    @endif
    ...
    <div class="bg-white rounded-lg shadow-lg w-3/5 p-6" style="width:700px;height:450px"> {{-- Medium size modal --}}
        <h3 id="modalTitle" class="text-lg font-bold mb-4">Add Luggage Guidelines</h3>
    ...
    <div class="mb-4">
        <label for="type" class="block text-sm font-medium text-gray-700">Luggage Type</label>
        <input type="text" name="type" id="type" class="w-full border-gray-300 rounded mt-1">
    </div>

    <div class="mb-4">
        <label for="max_dimensions_cm" class="block text-sm font-medium text-gray-700">Max Dimensions Inches</label>
        <input type="text" name="max_dimensions_cm" id="max_dimensions_cm" class="w-full border-gray-300 rounded mt-1">
    </div>

    <div class="mb-4">
        <label for="max_dimensions_inches" class="block text-sm font-medium text-gray-700">Max Dimensions Cm</label>
        <input type="text" name="max_dimensions_inches" id="max_dimensions_inches" class="w-full border-gray-300 rounded mt-1">
    </div>
...
function initSortable() {
    ...
    const currentPage = {{ $LuggageGuidelines->currentPage() }};
    const perPage = {{ $LuggageGuidelines->perPage() }};
    ...
    sortableInstance = new Sortable(tbody, {
        ...
        onEnd: function () {
            ...
            fetch("{{ route('LuggageGuideline.sort') }}", {
                method: "POST",
                headers: {
...
destroySortable();
 ...
 <script>
    ...
    function openEditModal(LuggageGuideline) {
    const modal = document.getElementById('createCarFeatureModal');
    const form = modal.querySelector('form');

    // Set form values from the car object
    form.querySelector("input[name='id']").value = LuggageGuideline.id;
    form.querySelector("input[name='type']").value = LuggageGuideline.type;
    form.querySelector("input[name='max_dimensions_inches']").value = LuggageGuideline.max_dimensions_inches;
    form.querySelector("input[name='max_dimensions_cm']").value = LuggageGuideline.max_dimensions_cm;


    // Change modal title
    modal.querySelector("h3").innerText = "Edit Luggage Guidelines";
...
function openAddModal() {
    ...
    // Change modal title
    modal.querySelector("h3").innerText = "Add Luggage Guidelines";
...
document.querySelector("#createCarFeatureForm").addEventListener("submit", function(e) {
    ...
    let url = "{{ route('LuggageGuideline.store') }}";
