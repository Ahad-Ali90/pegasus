...
<head>
  ...
  <title>{{$car->name}} – Pegasus Transfers</title>
...
.card-bg {
  ...
  height: 100%;
  width: 100%;
  background-size: contain;        /* ✅ fit full car inside */
...
<div class="container">
  ...
  <a href="{{route('index')}}">← Back to Home</a>
    ...
    <h1 style="  font-family: 'Playfair Display', serif;">{{$car->name}}</h1>

    <img src="{{ asset('storage/' . $car->main_image) }}" alt="{{$car->name}}" class="car-image" />
...
<div class="features">
  ...
  @endforeach
    ...
    <!-- FULL-WIDTH LUGGAGE REFERENCE SECTION -->
...
<p>Compare your bags to the standard size guidelines below:</p>
    ...
    <table class="luggage-table">
      ...
      <tbody>
      @foreach($car->luggageGuideline as $guideline)
        <tr>
          <td>{{$guideline->type}}</td>
          <td>{{$guideline->max_dimensions_cm}}</td>
          <td>{{$guideline->max_dimensions_inches}}</td>
        </tr>
        @endforeach
...
@foreach($car->luggageOption as $options)
