<x-app-layout>
  ...
  <span class="slider">
   <svg class="slider-icon" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="presentation"><path fill="none" d="m4 16.5 8 8 16-16"></path></svg> 
    ...
    {{-- Right side Add LuggageOption button --}}
    <div>
        <button onclick="openAddModal()"
            ...
            + Add LuggageOption
                    ...
                    <div class="overflow-x-auto bg-white shadow-sm sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200 border"  style="width:100%">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-center" id="sortableBody">
                                @foreach($LuggageOptions as $index => $LuggageOption)
                                    <tr class="text-center" data-id="{{ $LuggageOption->id }}">
                                        <td class="px-6 py-2">{{ $loop->iteration }}</td>
                                        <td class="px-6 py-2">{{ $LuggageOption->car->name }}</td>
                                        <td class="px-6 py-2">{{ $LuggageOption->title }}</td>
                                        <td class="px-6 py-2">{{ $LuggageOption->description }}</td>
                                        ...
                                        <button 
                                            type="button"
                                            onclick='openEditModal(@json($LuggageOption))'
                                            class="px-3 py-1 text-sm text-black bg-green-500 rounded hover:bg-green-600">
                                            Edit
                                        </button>
                                        {{-- Delete Button --}}
                                        <form action="{{route('LuggageOption.destroy',$LuggageOption->id)}}" 
                                            method="post" 
                                            class="inline-block ajax-delete-form"
                                            data-id="{{ $LuggageOption->id }}">
                                            @csrf
                                            @method('DELETE') 
                    ...
                    @if ($LuggageOptions->hasPages())
                        <div class="mt-4 bg-white p-4 rounded shadow">
                            {{ $LuggageOptions->links() }}
                        </div>
                    @endif
    ...
    <div id="createLuggageOptionModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center modal-bg-color" >
    <div class="bg-white rounded-lg shadow-lg w-3/5 p-6" style="width:700px;height:370px"> {{-- Medium size modal --}}
        <h3 id="modalTitle" class="text-lg font-bold mb-4">Add Luggage Guidelines</h3>
        <form id="createLuggageOptionForm">
    @csrf
    ...
    <div class="flex justify-end">
        <button type="button" onclick="closeModal('createLuggageOptionModal')" class="px-4 py-2 bg-gray-300 rounded mr-2 danger-btn">
            Cancel
        </button>
...
function initSortable() {
    ...
    const currentPage = {{ $LuggageOptions->currentPage() }};
    const perPage = {{ $LuggageOptions->perPage() }};
    ...
    sortableInstance = new Sortable(tbody, {
        ...
        onEnd: function () {
            ...
            fetch("{{ route('LuggageOption.sort') }}", {
                method: "POST",
                headers: {
...
destroySortable();
 ...
 <script>
    const modal = document.getElementById('createLuggageOptionModal');
    function openEditModal(LuggageOption) {
    const modal = document.getElementById('createLuggageOptionModal');
    const form = modal.querySelector('form');

    // Set form values from the car object
    form.querySelector("input[name='id']").value = LuggageOption.id;
    form.querySelector("input[name='title']").value = LuggageOption.title;
    form.querySelector("input[name='description']").value = LuggageOption.description;


    // Change modal title
    modal.querySelector("h3").innerText = "Edit Luggage Guidelines";

    openModal('createLuggageOptionModal');
}
...
document.querySelector("#createLuggageOptionForm").addEventListener("submit", function(e) {
    e.preventDefault();
    ...
    let url = "{{ route('LuggageOption.store') }}";
    ...
    .then(async res => {
        ...
        .then(data => {
            if (data.success) {
                ...
                playSound("success");
                closeModal('createLuggageOptionModal');
                refreshCurrentPage();
            } else {
